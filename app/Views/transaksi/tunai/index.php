...
<script>
    ...
    function tambahKeranjang() {
        $('#btnTambahKeranjang').click(() => {
            let id_produk = $('#namaProduk').val();
            let quantity_produk = $('#quantityProduk').val();
            $.ajax({
    ...
    function cekSubtotal() {
        ...
        let quantity = $("#quantityProduk").val()
        if(produk !='' && quantity !='') {
            $.ajax({
                ...
                success: (response) => {
                    $("#subtotalProduk").val(`Rp. ${rupiah(response.data)}`)
                    let cekStok = response.produk.stok_produk - quantity;
                    if(cekStok < 0) {
                        $('#quantityProduk').addClass('is-invalid');
                        $('#quantityValidasi').html(`Maaf, ${response.produk.nama_produk} hanya tersisa ${response.produk.stok_produk} ${response.produk.satuan_produk}`);
                        $('#btnTambahKeranjang').attr('disabled');
                        $('#btnTambahKeranjang').prop('disabled', true);
                    } else {
                        $('#quantityProduk').removeClass('is-invalid');
